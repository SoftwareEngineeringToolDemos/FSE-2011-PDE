...
namespace Tum.PDE.ToolFramework.Modeling.Visualization.ViewModel.Messaging.Events
{
    ...
    /// </remarks>
    public class ModelElementLinkDeletedEvent : ModelElementEvent<ElementDeletedEventArgs>
    {
        ...
        public void Subscribe(DomainRelationshipInfo domainRelationshipInfo, bool bSourceRole, Guid parentElementId, Action<ElementDeletedEventArgs> action)
        {
        ...
        public override void Publish(ElementDeletedEventArgs payload)
        {
            ...
            List<Action<object[]>> executionStrategies = PruneAndReturnStrategies(payload as ElementDeletedEventArgs);
        ...
        public void Unsubscribe(DomainRelationshipInfo domainRelationshipInfo, bool bSourceRole, Guid parentElementId, Action<ElementDeletedEventArgs> action)
        {
            #region dictionarySource
            if (bSourceRole)
                lock (dictionarySource)
                {
                    ...
                    if (dictionarySource.Keys.Contains(domainClassId))
                    {
                        if (dictionarySource[domainClassId].Keys.Contains(parentElementId))
                        {
                            IEventSubscription eventSubscription = dictionarySource[domainClassId][parentElementId].Cast<EventSubscription<ElementDeletedEventArgs>>().FirstOrDefault(evt => evt.Action == action);
            ...
            if (!bSourceRole)
                lock (dictionaryTarget)
                {
                    ...
                    if (dictionaryTarget.Keys.Contains(domainClassId))
                    {
                        if (dictionaryTarget[domainClassId].Keys.Contains(parentElementId))
                        {
                            IEventSubscription eventSubscription = dictionaryTarget[domainClassId][parentElementId].Cast<EventSubscription<ElementDeletedEventArgs>>().FirstOrDefault(evt => evt.Action == action);
                            if (eventSubscription != null)
                            {
