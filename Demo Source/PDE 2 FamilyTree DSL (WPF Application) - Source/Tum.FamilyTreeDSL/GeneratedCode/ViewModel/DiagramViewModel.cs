...
{
	/// <summary>
	/// This class represents the default diagram surface view model of the FamilyTreeDSL.
	/// 
	/// Double-derived class to allow easier code customization.
	/// </summary>
	public partial class FamilyTreeDSLDesignerDiagramSurfaceViewModel : FamilyTreeDSLDesignerDiagramSurfaceViewModelBase
	{
		#region Constructor
		/// <summary>
        /// Constuctor.
		...
		/// <param name="modelContext">Model context.</param>
        public FamilyTreeDSLDesignerDiagramSurfaceViewModel(FamilyTreeDSLViewModelStore viewModelStore, DslEditorDiagrams::Diagram diagram, DslEditorModeling::ModelContext modelContext)
        ...
        public FamilyTreeDSLDesignerDiagramSurfaceViewModel(FamilyTreeDSLViewModelStore viewModelStore, DslEditorDiagrams::Diagram diagram, DslEditorModeling::ModelContext modelContext, DslEditorModeling::ModelContext parentModelContext)
	...
	/// </summary>
	public abstract partial class FamilyTreeDSLDesignerDiagramSurfaceViewModelBase : DslEditorViewDiagrams::DiagramSurfaceViewModel
	{
		...
		/// <param name="modelContext">Model context.</param>
		/// <param name="parentModelContext">Parent model context.</param>
        protected FamilyTreeDSLDesignerDiagramSurfaceViewModelBase(FamilyTreeDSLViewModelStore viewModelStore, DslEditorDiagrams::Diagram diagram, DslEditorModeling::ModelContext modelContext, DslEditorModeling::ModelContext parentModelContext)
            : base(viewModelStore, diagram, modelContext)
        {
			...
			if( parentModelContext == null )
			{
				foreach(DslEditorModeling::BaseModelContext modelContext in this.ModelData.AvailableModelContexts)
				{
					if( modelContext.Name == "DefaultContext")
					{
						...
						//parentModelContext = modelContext as Tum.FamilyTreeDSL.DefaultContextModelContext;
						break;
					}
				}
			}
		...
		{
			return Tum.FamilyTreeDSL.DesignerDiagram.DomainClassId;
		}
        ...
        public override bool CanCreateRelationship(DslEditorViewDiagrams::ViewModelRelationshipCreationInfo info)
		{
			...
			if( !bCanCreate )
				bCanCreate = CanCreateRelationshipShapeParentOfShape(info);

			if( !bCanCreate )
				bCanCreate = CanCreateRelationshipShapeMarriedToShape(info);
        ...
        {
			if( shapeToCreate == global::Tum.FamilyTreeDSL.ParentOfShape.DomainClassId)
			{
				using(DslModeling::Transaction transaction = this.Store.TransactionManager.BeginTransaction("Add global::Tum.VModellXT.Dynamik.UebergangShape ") )
				{
					...
					CreateRelationship<global::Tum.FamilyTreeDSL.ParentOf>(info.Source.ShapeElement.Element, info.Target.ShapeElement.Element);
					
					transaction.Commit();
				}
			}
			if( shapeToCreate == global::Tum.FamilyTreeDSL.MarriedToShape.DomainClassId)
			{
				using(DslModeling::Transaction transaction = this.Store.TransactionManager.BeginTransaction("Add global::Tum.VModellXT.Dynamik.UebergangShape ") )
				{
					System.Collections.ObjectModel.ReadOnlyCollection<global::Tum.FamilyTreeDSL.MarriedTo> allInstances =  DslModeling::DomainRoleInfo.GetElementLinks<global::Tum.FamilyTreeDSL.MarriedTo>(info.Source.Element, global::Tum.FamilyTreeDSL.MarriedTo.HusbandDomainRoleId);
					if(allInstances.Count > 0 )
					{
						allInstances[0].Delete();
					}
			
					CreateRelationship<global::Tum.FamilyTreeDSL.MarriedTo>(info.Source.ShapeElement.Element, info.Target.ShapeElement.Element);
